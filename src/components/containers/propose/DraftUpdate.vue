			...
			>
				<div class="display-flex flex-column px-5 py-2">
					<p class="font-weight-bold mb-0 font-size-14 text-blur-color">
						Chỉnh sửa đề xuất: {{ currentData ? currentData.name : '' }}
					</p>
			...
			async handlerUpdatePropose() {
				...
				const fields = this.cateObj.fields.map((o: any) => {
					...
					if (o.type && o.type.id === 4) {
						...
						const firstRow = o.field_values_columns_body[0].map(
							(o: Record<string, any>) => o.txt
						);
						if (!firstRow.join('')) {
						...
						o.type &&
						(o.type.id === 1 || o.type.id === 2 || o.type.id === 6)
					) {
						obj.field_values = o.field_values;
						...
						if (
							!o.field_values_date.join('') ||
							!o.field_values_time.join('')
						) {
							obj.field_values = null;
						} else {
				...
				if (!inValid) {
					...
					const params = {
						name: this.cateObj.name,
						description: this.description,
						category_id: this.cateObj.id,
						field_values: Object.fromEntries(xp),
						approver_ids: this.cateObj.approvers.length
							? this.cateObj.approvers.map((o: any) => o.id)
							: [],
						follower_ids: [this.auth.user.user.id],
					};
					this.setMessageErr('');
					this.setLoadingBtn(true);
					const res = await api.propose.update(this.currentData?.id, params);
