<template>
  <v-data-table
    ...
    ref="tableMod"
    class="
      table-modify-css table-approval-model table-scroll-y
  ...
  >
    ...
    <template v-slot:item.code="{ item }">
      <div class="w-max-content">
        <a @click="detailItem(item)">{{ item.code }}</a>
      </div>
    </template>
...
<style lang="scss">
.v-data-table.table-approval-model {
  max-width: 100%;
}
.v-data-table.table-approval-model table thead tr {
  .color-box {
    min-width: 23px;
    padding-left: 0px;
  }
...
.v-data-table.table-approval-model table tbody tr td {
