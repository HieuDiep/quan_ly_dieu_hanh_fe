   ...
   <tr class="filter-prepend-body" :style="`top:${endedThead}px;position:sticky;z-index:3`">
    ...
    <template v-slot:item.actions="{ item }">
      <div class="w-max-content">
        ...
        <v-tooltip v-if="item.status && item.status.id === 1" bottom>
          <template v-slot:activator="{ on, attrs }">
            <img
              v-bind="attrs"
              v-on="on"
...
<style lang="scss">
.v-data-table.table-propose-model {
  max-width: 100%;
}
.v-data-table.table-propose-model table thead tr {
  .color-box {
    min-width: 23px;
    padding-left: 0px;
  }
...
.v-data-table.table-propose-model table tbody tr td {
