...
<script>
	...
	export default {
		...
		computed: {
			...
			listTicketLogsComputed() {
				const arr = [...this.listTicketLogs];
				arr.forEach(o => {
		...
		methods: {
			...
			handleLoadmoreComment() {
				...
				this.getAllListLogs(this.currentId, false);
			},
			...
			async getAllListLogs(id, scrollAble = true) {
				...
				const res = await api.approval.getListLogs(id, params);
				...
				try {
					if (res.status > 199 && res.status < 399) {
						this.listTicketLogs = res.data.data.logs;
						this.$nextTick(() => {
							if (scrollAble) {
								this.scrollToBtm();
							}
