				...
				<div class="content-block-top">
					...
					<div class="list-preview-files">
						...
						>
							...
							/>
							<a
								class="block-file pl-2 font-size-12"
								:href="i.url"
								target="_blank"
								>{{ i.name }}</a
							>
	...
	export default {
		...
		methods: {
			...
			async handleRemoveComment() {
				const res = await api.task.deleteComment(
					this.currentId,
					this.comment.id
				);
				if (!res) {
			...
			async handleEditComment() {
				...
				const res = await api.task.updateComment(
					this.currentId,
					this.comment.id,
					formData
				);
				...
				try {
					if (res.status > 199 && res.status < 399) {
						this.endEdit();
						this.afterUpdate(res.data.data.task_comment);
					} else {
						// messageErr.value = res.data.data.error || res.data.message;
						this.$store.commit('SET_SNACKBAR', {
							type: 'error',
